          ...
          >
            ...
            {dropdownOpen && (
              <div className="absolute right-0 top-full pt-1.5 w-72 z-50 animate-in fade-in slide-in-from-top-3 duration-200" id="user-dropdown-menu-wrapper">
                ...
                {/* Mobile Simulator Toggle inside dropdown (Moved from Header Top Bar and renamed to "모바일") */}
                <div className="border-b border-neutral-800/80 pb-2 mb-1.5">
                  <button
                    onClick={() => {
                      onToggleSimulator();
                    ...
                    className={`w-full py-2 px-3 rounded-lg border text-xs font-bold transition-all flex items-center justify-center gap-2 cursor-pointer ${
                      isMobileSimulated
                        ...
                        : 'bg-kpcia-gold/10 text-kpcia-gold border-kpcia-gold/25 hover:bg-kpcia-gold/20'
                    }`}
                    id="dropdown-mobile-toggle-btn"
                  >
                    <Smartphone className="w-3.5 h-3.5 text-kpcia-gold animate-pulse" />
                    <span>{isMobileSimulated ? '웹사이트 보기' : '모바일'}</span>
                  </button>
                </div>
